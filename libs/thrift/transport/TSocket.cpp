...
//Not tested, should probably not work
...
//"Working", no support for connection timeouts and probably other things
...
retry_poll:
  ...
  if (ret > 0) {
    ...
    // emscripten has the extra errorcode EINPROGRESS which indicates, that
    // the websocket connection is not established yet.
    // When EINPROGRESS occurs, poll returns 0 instead of -1.
    // If we get that errorcode, we retry the poll, until
    // the connection is established.
    if(THRIFT_GET_SOCKET_ERROR == THRIFT_EINPROGRESS){
	  //THRIFT_GET_SOCKET_ERROR = 0;
      emscripten_sleep(1);
      goto retry_poll;
    }
#endif
...
//Working, unix sockets not testest, those should not work
void TSocket::open() {
  if (isOpen()) {
    return;
  }
...
//Not working, because unix sockets probably dont work in browsers
...
#ifdef __EMSCRIPTEN__
  // remove AI_ADDRCONFIG flag, when compiling with emscripten, because
  // it causes a crash
...
//Working / At least not crashing
void TSocket::close() {
  if (socket_ != THRIFT_INVALID_SOCKET) {
#ifndef __EMSCRIPTEN__
	//Shutdown is not implemented in emscripten
...
//Working / Timeout and retry not tested, but behavior was already weird previously
...
#ifdef __EMSCRIPTEN__
  // when compiled with emscripten sockets are always nonblocking, so we
  // need to keep track of a timed out connection ourselves.
  int32_t emscriptenRemainingTimeout = recvTimeout_ * 1000;
#endif
...
try_again:
  ...
  if (got < 0) {
    if (errno_copy == THRIFT_EAGAIN) {
      ...
      if (recvTimeout_ == 0) {
#ifdef __EMSCRIPTEN__
        // in emscripten sockets are always nonblocking,
        // so we wait a little and try again, to simulate blocking behaviour
        emscripten_sleep(1);
        goto try_again;
#else
...
#ifdef __EMSCRIPTEN__
      // if the timeout is not exceeded, we sleep before we retry.
      if(emscriptenRemainingTimeout > 0){
		// increase sleep duration to keep load low
		//TODO determine if increasing sleep duration makes sense
	    uint32_t duration = 0;
	    if(emscriptenRemainingTimeout < recvTimeout_ * 950){
		duration = 100;
		}else if(emscriptenRemainingTimeout < recvTimeout_ * 990){
		duration = 10;
		}else if(emscriptenRemainingTimeout < recvTimeout_ * 995){
		duration = 1;
		}
...
#ifdef __EMSCRIPTEN__
      // if the timeout is not exceeded, we retry, to simulate blocking behaviour.
      if(emscriptenRemainingTimeout > 0){
		emscriptenRemainingTimeout -= readElapsedMicros;
        goto try_again;
      }
#endif
...
#ifdef __EMSCRIPTEN__
  //Emscripten only supports nonblocking Operation
  flags |= MSG_DONTWAIT;
#endif
...
#ifdef __EMSCRIPTEN__
    if (errno_copy == THRIFT_EINPROGRESS){
		throw TTransportException(TTransportException::NOT_OPEN, "write() send()", errno_copy);
	}
#endif
...
//Working / probably depends on whether opname is supported
...
//Working / could need timeout to be nonzero
void TSocket::setSendTimeout(int ms) {
  setGenericTimeout(socket_, ms, SO_SNDTIMEO);
  sendTimeout_ = ms;
}
//Should work / Not crashing with true and false
